...
def main() -> int:
    ...
    exit_code = 1
    json_path = None
    publish_json_path = None
    try:
        logger.info("scheduled snapshot start mode=%s started_at=%s write_drive=%s publish_after_success=%s log_path=%s", args.mode, started_at.isoformat(), args.write_drive, args.publish_after_success, log_path)
        ...
        logger.info("scheduled snapshot success mode=%s json_path=%s", args.mode, json_path or "")
        if args.publish_after_success:
            ...
            publish_json_path = publish_result.get("json_path")
            logger.info("publish success mode=%s publish_json_path=%s deploy_branch=%s", args.mode, publish_result.get("json_path", ""), publish_result.get("deploy_branch", ""))
        print(f"scheduled snapshot completed mode={args.mode} json_path={json_path or ''}")
        exit_code = 0
        return 0
    except Exception:
        if args.publish_after_success and json_path is not None:
