...
def bundle_to_json_ready(bundle: dict[str, Any]) -> dict[str, Any]:
    payload = {str(key): _json_ready(value) for key, value in bundle.items()}
    if "sector_summary" not in payload:
        payload["sector_summary"] = _json_ready(_frame_from_bundle(bundle, "today_sector_leaderboard", "today_sector_summary"))
    if "leaders_by_sector" not in payload:
        payload["leaders_by_sector"] = _json_ready(
            _frame_from_bundle(bundle, "sector_representatives", "sector_representatives_display")
        )
    if "focus_candidates" not in payload:
        payload["focus_candidates"] = _json_ready(
            _frame_from_bundle(bundle, "swing_candidates_1w", "swing_candidates_1w_display")
        )
    return payload
...
def bundle_to_markdown(bundle: dict[str, Any]) -> str:
    sector_summary = _frame_from_bundle(bundle, "sector_summary", "today_sector_leaderboard", "today_sector_summary")
    leaders_by_sector = _frame_from_bundle(
        bundle,
        "leaders_by_sector",
        "sector_representatives",
        "sector_representatives_display",
    )
    focus_candidates = _frame_from_bundle(
        bundle,
        "focus_candidates",
